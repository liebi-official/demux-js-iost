...
export class BaseActionWatcher {
  ...
  public async watch(isReplay: boolean = false) {
    ...
    this.log.debug(
      `Block check took ${duration}ms; waiting ${waitTime}ms before next check`
    );
  ...
  protected async checkForBlocks(isReplay: boolean = false) {
    ...
    while (
      !headBlockNumber ||
      this.actionReader.info.currentBlockNumber < headBlockNumber
    ) {
      if (this.shouldPause) {
        this.processIntervals = [];
        return;
      ...
      this.log.debug(
        `Processing block ${this.actionReader.info.currentBlockNumber + 1}...`
      );
  ...
  private addProcessInterval(start: number, end: number) {
    ...
    if (this.processIntervals.length > this.velocitySampleSize) {
      this.processIntervals.splice(
        0,
        this.processIntervals.length - this.velocitySampleSize
      );
    }
  }
  ...
  private getMaxBlockVelocity(): number {
    ...
    const processTimes = this.processIntervals.map(
      ([start, end]) => end - start
    );

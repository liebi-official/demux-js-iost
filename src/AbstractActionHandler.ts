...
export abstract class AbstractActionHandler implements ActionHandler {
  ...
  public async handleBlock(
    ...
    if (!this.initialized) {
      this.log.info(
        "Action Handler was not initialized before started, so it is being initialized now"
      );
    ...
    if (
      ...
      this.log.debug(
        `Block ${blockInfo.blockNumber} was just handled; skipping`
      );
      return null;
    }
  ...
  protected async applyUpdaters(
    ...
    const {
      block: { actions, blockInfo },
    } = nextBlock;
    for (const action of actions) {
      let updaterIndex = -1;
      for (const updater of this.handlerVersionMap[this.handlerVersionName]
        .updaters) {
        updaterIndex += 1;
        if (
          this.matchActionType(action.type, updater.actionType, action.payload)
        ) {
          const { payload } = action;
          this.log.debug(
            `Applying updater for action type '${action.type}'...`
          );
          const updaterStart = Date.now();
          const newVersion = await updater.apply(
            state,
            payload,
            blockInfo,
            context
          ...
          this.log.debug(
            `Applied updater for action type '${action.type}' (${updaterTime}ms)`
          );
          if (
            newVersion &&
            !this.handlerVersionMap.hasOwnProperty(newVersion)
          ) {
            this.warnHandlerVersionNonexistent(newVersion);
          } else if (newVersion) {
            this.log.info(
              `Updated Handler Version to '${newVersion}' (block ${blockInfo.blockNumber})`
            );
            this.warnSkippingUpdaters(updaterIndex, action.type);
            await this.loggedUpdateIndexState(
              state,
              nextBlock,
              isReplay,
              newVersion,
  ...
  protected async handleActions(
    ...
    const versionedActions = await this.applyUpdaters(
      state,
      nextBlock,
      context,
      isReplay
  ...
  private async handleRollback(
    ...
    if (isRollback || (isReplay && isEarliestBlock)) {
      ...
      this.log.debug(
        `Rolling back ${rollbackCount} blocks to block ${rollbackBlockNumber}...`
      );
  ...
  protected async runOrDeferEffect(
    ...
    const curriedEffectRun = this.curryEffectRun(
      effect,
      payload,
      blockInfo,
      context,
    ...
    const shouldRunImmediately =
      !effect.deferUntilIrreversible ||
      blockInfo.blockNumber <= lastIrreversibleBlockNumber;
    if (shouldRunImmediately) {
      this.runningEffects.push(
        makeQuerablePromise(
          curriedEffectRun(blockInfo.blockNumber, true),
          false
        )
      );
    } else {
  ...
  protected shouldRunOrDeferEffect(effect: Effect, action: Action) {
    if (!this.matchActionType(action.type, effect.actionType, action.payload)) {
      ...
      this.effectRunMode === EffectRunMode.OnlyImmediate &&
      effect.deferUntilIrreversible
    ) {
      return false;
    } else if (
      this.effectRunMode === EffectRunMode.OnlyDeferred &&
      !effect.deferUntilIrreversible
    ) {
      return false;
    }
  ...
  private curryEffectRun(
    ...
    return async (currentBlockNumber: number, immediate: boolean = false) => {
      ...
      this.log.debug(
        `Running ${immediate ? "" : "deferred "}effect for '${
          effect.actionType
        }'...` +
      ...
      this.log.debug(
        `Ran ${immediate ? "" : "deferred "}effect for '${
          effect.actionType
        }' (${effectTime}ms)`
      );
    };
  ...
  private async runDeferredEffects(lastIrreversibleBlockNumber: number) {
    ...
    for (const blockNumber of this.range(
      nextDeferredBlockNumber,
      lastIrreversibleBlockNumber + 1
    )) {
      if (this.deferredEffects[blockNumber]) {
        const effects = this.deferredEffects[blockNumber];
        this.log.debug(
          `Block ${blockNumber} is now irreversible, running ${effects.length} deferred effects`
        );
  ...
  private getNextDeferredBlockNumber() {
    const blockNumbers = Object.keys(this.deferredEffects).map((num) =>
      parseInt(num, 10)
    );
  ...
  private checkRunningEffects(): EffectsInfo {
    ...
    this.effectErrors.splice(
      0,
      this.effectErrors.length - this.maxEffectErrors
    );
    this.runningEffects = this.runningEffects.filter((effectPromise) =>
      effectPromise.isPending()
    );
    return {
  ...
  private warnMissingHandlerVersion(actualVersion: string) {
    this.log.warn(
      `No Handler Version found with name '${this.handlerVersionName}': starting with ` +
  ...
  private warnIncorrectFirstHandler(actualVersion: string) {
    this.log.warn(
      `First Handler Version '${actualVersion}' is not '${this.handlerVersionName}', ` +
  ...
  private warnHandlerVersionNonexistent(newVersion: string) {
    this.log.warn(
      `Attempted to switch to handler version '${newVersion}', however this version ` +
  ...
  private warnSkippingUpdaters(updaterIndex: number, actionType: string) {
    const remainingUpdaters =
      this.handlerVersionMap[this.handlerVersionName].updaters.length -
      updaterIndex -
      1;
    if (remainingUpdaters) {
      this.log.warn(
        `Handler Version was updated to version '${this.handlerVersionName}' while there ` +

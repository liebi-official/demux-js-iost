...
const defaultBlock: Block = {
  blockInfo: {
    ...
    headParentHash: "", // block parent head hash
...
export abstract class AbstractActionReader implements ActionReader {
  ...
  protected async resolveFork() {
    ...
    while (this.blockHistory.length > 0) {
      ...
      this.log.info(
        `Refetching Block ${this.currentBlockData.blockInfo.blockNumber}...`
      );
    ...
    this.currentBlockNumber =
      this.blockHistory[this.blockHistory.length - 1].blockInfo.blockNumber + 1;
  }
  ...
  private async getLatestNeededBlockNumber() {
    if (this.onlyIrreversible) {
      ...
      this.log.debug(
        `Got head block number: ${headBlockNumber} (${headBlockFetchTime}ms)`
      );
  ...
  private async reloadHistory(maxTries = 10) {
    ...
    while (microForked) {
      ...
      for (const blockNumber of historyRange) {
        ...
        const latestHistoryBlockHash = this.blockHistory[
          this.blockHistory.length - 1
        ].blockInfo.blockHash;
        if (
          latestHistoryBlockHash !== historyBlock.blockInfo.previousBlockHash
        ) {
          this.log.info("Microforked while reloading history!");
          this.log.info(`  EXPECTED: ${latestHistoryBlockHash}`);
          this.log.info(
            `  RECEIVED: ${historyBlock.blockInfo.previousBlockHash}`
          );
          this.log.info(
            `Scrapping history and trying again (try ${tryCount + 1})`
          );
          microForked = true;
          break;
        }
  ...
  private async addPreviousBlockToHistory(
    checkIrreversiblility: boolean = true
  ) {
    if (
      this.currentBlockData.blockInfo.blockNumber <
        this.lastIrreversibleBlockNumber &&
      checkIrreversiblility
    ) {
  ...
  private async loggedGetBlock(
    ...
    this.log.debug(
      `Got block ${blockNumber} (${getBlockFetchTime}ms; ${block.actions.length} actions)`
    );
    return block;
  }

  private logForkDetected(
    unvalidatedBlockData: Block,
    expectedHash: string,
    actualHash: string
  ) {
    ...
    this.log.info(
      `    ✓ NEW Block ${unvalidatedBlockData.blockInfo.blockNumber} previous: ${actualHash}`
    );
    this.log.info(
      `    ✕ OLD Block ${this.currentBlockNumber} id:       ${expectedHash}`
    );
  }
  ...
  private logForkResolved(
    ...
    this.log.info("  MATCH:");
    this.log.info(
      `    ✓ NEW Block ${currentBlockInfo.blockNumber} previous: ${currentBlockInfo.previousBlockHash}`
    ); // tslint:disable-line
    this.log.info(
      `    ✓ OLD Block ${previousBlockInfo.blockNumber} id:       ${previousBlockInfo.blockHash}`
    );
  ...
  private logForkMismatch(
    ...
    this.log.info(
      `    ✕ OLD Block ${previousBlockInfo.blockNumber} id:       ${previousBlockInfo.blockNumber}`
    );
  }
}
